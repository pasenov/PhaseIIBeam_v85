...
StackingAction::ClassifyNewTrack(const G4Track* track)
{
  ...
  //geometry parameters
  G4double Det1SizeZ, Det2SizeZ, Dist, Strip1Depth, Strip1Length, Strip2Depth, Strip2Length, StripDist, StripWidth, xAbs, XangleDUT, XangleBPIX, YangleBPIX, theta;
  ...
  if (track->GetParentID() == 1)  {
    ...
    if (volume == fDetectorconstruction->GetWorld()) {
      ...
      if (z_prim < posZDUT - Dist/2 - Det1SizeZ)  {
      ...
      if ((z_prim > posZDUT - Dist/2) && (z_prim < posZDUT + Dist/2))  {
      ...
      if (z_prim > posZDUT + Dist/2 + Det2SizeZ)  {
    ...
    if (volume == fDetectorconstruction->GetDet1()) {
      ...
      /*G4cout
         << "\n particle from 2S, outside sensors"
         << G4endl;*/
      ...
      if(particleDefinition == G4Electron::Definition())   {
        ...
        fEventaction->AddNb2SElectrons();
        /*G4cout
            << "\n electron from 2S"
            << G4endl;*/
	if ((z_prim >= posZDUT - Dist/2 - Det1SizeZ) && (z_prim <= posZDUT - Dist/2 - Det1SizeZ + Strip1Depth) && (x_prim >= (-Strip1Length/2)) && (x_prim <= Strip1Length/2)) {
            /*G4cout
               << "\n electron from 2S"
               << G4endl; */
